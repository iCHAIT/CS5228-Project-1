...
submission_filepath = os.path.join(data_dir, 'validation_labels_random_forest.csv')
write_to_csv_file(filepath=submission_filepath, data=validation_pred)

print('Random Forest - Acc in training set %5.3f, in validation set %5.3f; MCC in training set %5.3f, in validation set %5.3f'
	% (train_accuracy, validation_accuracy, mcc_train,mcc_validation) )


test_labels = clf.predict(X_test)

submission_filepath = os.path.join(data_dir, 'test_labels_random_forest.csv')
